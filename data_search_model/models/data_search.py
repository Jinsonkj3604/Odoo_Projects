...
class DataSearchFiled(models.Model):
    ...
    model_name = fields.Many2many('ir.model', string="models", help="If its blank all models data will be print / Select your model")

    def search_action_confirm(self):

        model_list = []
        all_list = []
        records = self.env['data.search.model'].search([])
        models = records.mapped('model_name')
        for tag in models:
            model_list.append(tag.model)
        print(model_list)

        self.write({'content_ids': [(5, 0, 0)]})
        # selected model list
        models = ['res.partner', 'sale.order', 'product.product', 'product.template', 'sale.order.line','res.country','res.company','account.move']
